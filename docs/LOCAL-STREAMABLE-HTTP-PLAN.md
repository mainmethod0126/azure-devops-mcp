...
- `--http-session-idle-timeout-seconds <seconds>`: 기본값 `1800`
...
- 세션별로 `McpServer`와 transport를 생성하고 `MCP-Session-Id` 기준 메모리 맵으로 관리한다.
- 세션은 초기화 시 생성하고 마지막 활동 시각을 갱신한다.
- idle timeout 또는 `DELETE /mcp` 시 세션을 정리하며, 종료된 세션으로 다시 접근하면 `404`를 반환한다.
...
- 세션 테스트로 `initialize` 후 `tools/list` 호출, `DELETE` 이후 재호출 실패, idle timeout 만료 후 새 세션 재시작 흐름을 검증한다.
